...
typedef enum tagNETDEVAlarmTypeEn
{
    ...
    NETDEV_ALARM_DISK_OFFLINE                       = 9,            /* 磁盘下线 Disk offline */
    ...
    NETDEV_ALARM_DISK_OFFLINE_RECOVER               = 1009,         /* 磁盘离线恢复 Disk offline recover */
...
{
    BYTE *pucData;                 /**< 音频数据指针 */
    UINT32 udwDataLen;             /**< 音频数据长度 */
    UINT32 udwAudioCodeFormat;     /**< 音频编码格式，从# PLAYER_AUDIO_ENCODE_TYPE_E 中取值 */
    UINT32 udwWaveFormat;          /**< 音频数据解码后音频格式，对应# PLAYER_WAVE_FORMAT_INFO_E 枚举中的值 */
    INT64 tTimeStamp;              /**< 时间戳（毫秒）*/
...
* @param [IN]  cbPlayParseAudioCallBack    码流数据回调函数
...
* 注册码流回调函数：解码后音频媒体流数据\n
* @param [IN]  lpPlayHandle                 实时预览句柄
* @param [IN]  cbPlayDecodeAudioCallBack    码流数据回调函数
* @param [IN]  lpUserData                   用户数据
* @return NETDEV_E_SUCCEED表示成功，其他表示失败
* @note
*/
...
* @param [IN]  lpUserID                 用户ID  User ID
...
* @param [IN]  lpUserData               用户数据   User data
* @return 返回的用户登录ID,返回 0 表示失败，其他值表示返回的用户ID值 Returned user ID. 0 means failure, and any other value is a user ID.
* @note
*/
...
* @return 返回的用户登陆ID,-1表示失败，其他值表示返回的用户ID值  Returned user ID. 1 means failure, any other value is a user ID.
* @note
*/
...
typedef enum tagNetDevAudioSampleFormatType
{
    ...
    NETDEV_AUDIO_SAMPLE_FMT_U8      = 0,            /* 无符号位整型*/
    NETDEV_AUDIO_SAMPLE_FMT_S16     = 1,            /* 有符号位整型*/
    NETDEV_AUDIO_SAMPLE_FMT_S32     = 2,            /* 有符号位整型*/
...
{
    NETDEV_WAVE_FORMAT_PCM8M16        = 0,        /* PCM，K采样率，单声道（M），位采样*/
    NETDEV_WAVE_FORMAT_PCM16M16       = 1,        /* PCM，K采样率，单声道（M），位采样*/
    NETDEV_WAVE_FORMAT_PCM441M16      = 2,        /* PCM，.1K采样率，单声道（M），位采样*/
    NETDEV_WAVE_FORMAT_PCM11025M16    = 3,        /* PCM，.025K采样率，单声道（M），位采样*/
...
{
    INT32 dwChannels;                               /* 声道数，单声道为，立体声为*/
...
* @param [IN]  lpUserData               用户数据
* @return 返回的对讲句柄,返回0 表示失败，其他值表示返回的对讲句柄
* @note
*/
...
/**
* 设置解析后语音数据回调函数\n
* @param [IN]  lpPlayHandle                 预览\回放句柄Preview\playback handle
* @param [IN]  cbPlayDecodeVideoCALLBACK    数据回调函数Data callback function
* @param [IN]  bContinue                    是否继续Continue or not
* @param [IN]  lpUserData                   用户数据User data
* @return TRUE表示成功,其他表示失败TRUE means success, and any other value means failure.
* @note
* - 若关闭回调函数,将第二个参数置为NULL。
* - To shut the callback function, set the second parameter as NULL.
*/
