...
void Lox::SetVerbosity( Logger* logger, Verbosity verbosity, const String& domain, int priority )
{
    ...
    Domain *dom = evaluatdrillResultDomain(domain);
...
void Lox::Entry( const String& domain, Verbosity verbosity )
{
    ...
    log(evaluatdrillResultDomain(domain), verbosity, *logableContainers[static_cast<size_t>(cntAcquirements - 1)], Inclusion::Include);
}
...
// internals
// #################################################################################################
        Domain *Lox::evaluatdrillResultDomain(const String &domainPath)
{
    String128 resDomain;
